...
<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-col gap-3">
      <div class="flex gap-2">
        <input
          ...
          class="flex-1 px-4 py-3 rounded-xl glass-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0071e3]/50 transition text-sm border border-white/30 dark:border-white/10"
        />
        <select
          v-if="availableLanguages.length"
          :value="languageFilter"
          @change="emit('update:languageFilter', ($event.target as HTMLSelectElement).value)"
          class="px-3 py-2 rounded-xl glass-sm text-gray-700 dark:text-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-[#0071e3]/50 transition cursor-pointer border border-white/30 dark:border-white/10"
    ...
    <p v-else-if="!loading" class="text-gray-500 dark:text-gray-400 text-center py-10 text-sm font-medium">
